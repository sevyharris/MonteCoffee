...
class CODesEvent(EventBase):
    ...
    def get_rate(self, system, site, other_site):
        GCN = system.sites[site].GCN
        strain = system.sites[site].strain
        Ncovs = system.get_ncovs(site)
        ECO = max(EadsCO(GCN,strain) - get_repulsion(1, Ncovs, GCN), 0)
...
class ODesEvent(EventBase):
    ...
    def get_rate(self, system, site, other_site):
        GCN = system.sites[site].GCN
        strain = system.sites[site].strain
        GCN_other = system.sites[other_site].GCN
        Ncovs = system.get_ncovs(site)
        Ncovsother = system.get_ncovs(other_site)
        E2O = max(2. * EadsO(GCN,strain) - get_repulsion(1, Ncovs, GCN) \
                  - get_repulsion(1, Ncovsother, GCN_other), 0.)
...
class CODiffEvent(EventBase):
    ...
    def get_rate(self, system, site, other_site):
        ...
        strain = system.sites[site].strain
        GCN_other = system.sites[other_site].GCN
        strain_other = system.sites[other_site].strain
        ...
        E = max(0., EadsCO(GCN,strain) - get_repulsion(1, Ncovs, GCN))
        Eother = max(0, EadsCO(GCN_other,strain_other) - get_repulsion \
            (1, Nothercovs, GCN_other))
...
class ODiffEvent(EventBase):
    ...
    def get_rate(self, system, site, other_site):
        ...
        E = EadsO(GCN,strain) - get_repulsion(2, Ncovs, GCN)
        Eother = EadsO(GCN_other,strain_other) - get_repulsion \
            (2, Nothercovs, GCN_other)
...
class COOxEvent(EventBase):
    ...
    def get_rate(self, system, site, other_site):
        ...
        GCN_other = system.sites[other_site].GCN
        strain_other = system.sites[other_site].strain
        ECO = EadsCO(GCN, strain)
        EO = EadsO(GCN_other, strain_other)
        # Find the Nearest neighbor repulsion
        Ncovs = [system.sites[n].covered for n in
                 system.neighbors[site]]
        Nothercovs = [system.sites[n].covered for n
                      in system.neighbors[other_site]]
        ECO -= get_repulsion(1, Ncovs, GCN)
        EO -= get_repulsion(2, Nothercovs, GCN_other)
