...
class CODesEvent(EventBase):
    ...
    def get_rate(self, system, site, other_site):
        ...
        ECO = max(EadsCO(system.sites[site].stype) - get_repulsion(1, Ncovs, system.sites[site].stype), 0)
...
class ODesEvent(EventBase):
    ...
    def get_rate(self, system, site, other_site):
        ...
        E2O = max(EadsO(system.sites[site].stype) + EadsO(system.sites[other_site].stype) - get_repulsion(2, Ncovs, system.sites[site].stype) - get_repulsion(2, Ncovsother, system.sites[other_site].stype), 0.)
...
class CODiffEvent(EventBase):
    ...
    def get_rate(self, system, site, other_site):
        if system.sites[site].covered == 1:
            ...
            init_stype = system.sites[site].stype
            E = max(0., EadsCO(init_stype) - get_repulsion(1, Ncovs, init_stype))
            system.sites[site].covered = 0
            system.sites[other_site].covered = 1
            Nothercovs = [system.sites[n].covered for n in system.neighbors[other_site]]
            final_stype = system.sites[other_site].stype
            Eother = max(0, EadsCO(final_stype)- get_repulsion(1, Nothercovs, final_stype))
            system.sites[site].covered = 1 
            system.sites[other_site].covered = 0
            dE = max(E - Eother, 0.)
        else:
            Ncovs = [system.sites[n].covered for n in system.neighbors[other_site]]
            init_stype = system.sites[other_site].stype
            E = max(0., EadsCO(init_stype) - get_repulsion(1, Ncovs, init_stype))
            system.sites[site].covered = 1
            system.sites[other_site].covered = 0
            Nothercovs = [system.sites[n].covered for n in system.neighbors[site]]
            final_stype = system.sites[site].stype
            Eother = max(0, EadsCO(final_stype) - get_repulsion(1, Nothercovs, final_stype))
            system.sites[site].covered = 0 
            system.sites[other_site].covered = 1
            dE = max(E - Eother, 0.)
...
class ODiffEvent(EventBase):
    ...
    def get_rate(self, system, site, other_site):
        if system.sites[site].covered == 2:
            Ncovs = [system.sites[n].covered for n in system.neighbors[site]]
            init_stype = system.sites[site].stype
            E = max(0., EadsO(init_stype) - get_repulsion(2, Ncovs, init_stype))
            system.sites[site].covered = 0
            system.sites[other_site].covered = 2
            Nothercovs = [system.sites[n].covered for n in system.neighbors[other_site]]
            final_stype = system.sites[other_site].stype
            Eother = max(0, EadsO(final_stype) - get_repulsion(2, Nothercovs, final_stype))
            system.sites[site].covered = 2 
            system.sites[other_site].covered = 0
            dE = max(E - Eother, 0.)
        else:
...
class COOxEvent(EventBase):
    ...
    def get_rate(self, system, site, other_site):
        ...
        if system.sites[site].covered == 1:
            CO_stype = system.sites[site].stype
            ECO = EadsCO(CO_stype)
            O_stype = system.sites[other_site].stype
            EO = EadsO(O_stype)
            ...
            CO_stype = system.sites[other_site].stype
            ECO = EadsCO(CO_stype)
            O_stype = system.sites[site].stype
            EO = EadsO(O_stype)
        ...
        ECO -= get_repulsion(1, Ncovs_CO, CO_stype)
        EO -= get_repulsion(2, Ncovs_O, O_stype)
