...
class NeighborKMC(NeighborKMCBase):
    ...
    def run_kmc(self):
        ...
        print ('Real end time:', self.real_t_end)
        start_time = time.time()
        if os.path.exists('data.pck'):
            self.deserialize('data.pck')
            print ('load pickeled data')
            print (self.t, self.tend)
            self.save_atoms('reload_struct.traj')
    ...
    #            self.MCstep.append(stepNMC)
        ...
        self.serialize('data.pck')
        self.save_atoms('break_state.traj')
        print ('Real time is up')
        log.dump_point(self.stepNMC, self.t, self.evs_exec)
        self.save_txt()
